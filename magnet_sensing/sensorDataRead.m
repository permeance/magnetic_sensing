...
for sensor_step = 1:size(num_sensors)
    ...
    for list_item = 1:size(time_index)
        if sensor_step == (sensor_raw(list_item)+1) %save data per sensor 
            ...
            %Separates the X1,X2 into the bottom/top data
...
sensor_names = ["Sensor 0 Top", "Sensor 0 bottom", "Sensor 1 top", "Sensor 1 bottom",...
...
table_name = append(filename,'_data.xls');
